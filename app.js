...
const AUDIO_CONFIG = {
  ...
  alarm:      { path: 'Sound/Alarm.mp3', label: 'Alarm',      icon: '🚨', },
};
...
async function speakText(text){
  ...
  // TTS -> AudioContext -> MediaStreamDestination -> <audio setSinkId>
  // So landet die Stimme auf dem gewählten Ausgabegerät (wie beim Lautsprecher-Test)
  try{
    ...
    const outId = state.outputDeviceId;
    if(outId && typeof el.setSinkId === 'function'){
      try{ await el.setSinkId(outId); }catch(e){}
    }
    ...
    utter.onend = () => {
      removeActive('TTS');
      setTimeout(()=>{ try{el.remove(); ctx.close();}catch(e){} }, 300);
    };
    utter.onerror = (e) => {
      removeActive('TTS');
    ...
    // Fallback: direkt sprechen ohne Routing
    console.warn('TTS AudioContext routing failed, falling back', e);
    const utter = new SpeechSynthesisUtterance(text.trim());
    utter.lang = 'de-DE';
    utter.onstart = () => addActive('TTS');
...
async function startMic(){
  ...
  try{
    ...
    // Analyser fuer Visualisierung
    const analyser=audioCtx.createAnalyser(); analyser.fftSize=256;
    source.connect(analyser);

    // Ausgang: Mikrofon live auf gewaehltes Ausgabegeraet routen
    const dest=audioCtx.createMediaStreamDestination();
    source.connect(dest);
    const outEl=document.createElement('audio');
    outEl.autoplay=true;
    outEl.srcObject=dest.stream;
    if(state.outputDeviceId && typeof outEl.setSinkId==='function'){
      try{ await outEl.setSinkId(state.outputDeviceId); }catch(e){ console.warn('setSinkId failed',e); }
    }
    document.body.appendChild(outEl);
    state.micOutEl=outEl;
    ...
    addActive('Live-Mic');
  }catch(e){ alert('Mikrofonzugriff verweigert oder Geraet fehlt: '+e.message); }
}
function stopMic(){
  ...
  if(state.micOutEl){state.micOutEl.srcObject=null;state.micOutEl.remove();state.micOutEl=null;}
